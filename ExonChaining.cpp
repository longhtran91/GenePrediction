...
ExonChaining::ExonChaining(const list<Interval_Coordinate> &exons, QProgressDialog &p)
{
    ...
    unsigned int a = 0;
    unsigned int b = 50;
    unsigned int d = exons.size()-1;
    unsigned int c = 0.1*d;
    map<unsigned int, unsigned int> mprogress;
    for (unsigned int i = a; i <= b; ++i)
    {
        unsigned int r = c+(d-c)/(b-a)*(i-a);
        mprogress.insert({r, i});
    }
	for (const auto &elem : exons)
	{
        ...
        if (mprogress.count(numTasks) > 0)
        {
            p.setValue(mprogress.at(numTasks));
        }
        if (p.wasCanceled())
        {
    ...
    if (!cancelled)
    {
        ...
        for (unsigned int i = 1; i < size; ++i)
        {
            ...
            pre_j = right_j;
            if (mprogress.count(i) > 0)
            {
                p.setValue(mprogress.at(i));
            }
            if (p.wasCanceled())
            {
                this->intervals.clear();
    ...
    if (!cancelled)
    {
        ...
        while (index >= 0)
        {
            ...
            if (mprogress.count(index) > 0)
            {
                p.setValue(mprogress.at(index));
            }
            if (p.wasCanceled())
            {
                this->intervals.clear();
